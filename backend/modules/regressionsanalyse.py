...
class PersonEstimator:
    """Schaetzt Personenanzahl anhand des VOC-Werts (BME680 gas_resistance).
    Der RCWL-0516 Bewegungssensor dient als Plausibilitaetspruefung:
    Ohne erkannte Bewegung werden hohe VOC-Werte als Stoerquelle gedaempft.
    """
    ...
    def estimate(self, gas_resistance, movement_detected=False):
        """Berechnet Personenschaetzung aus aktuellem Gaswiderstand.
        Prinzip: Mehr Personen -> mehr VOC -> niedrigerer Gaswiderstand.
        Bewegungssensor: Plausibilitaetspruefung (ohne Bewegung -> Daempfung).
        """
        ...
        # Plausibilitaetspruefung: Ohne Bewegung aber hohe VOC -> wahrscheinlich Stoerquelle
...
class TemperatureRegression:
    ...
    Wird alle 48 Stunden mit den gesammelten Daten trainiert.
    """
    ...
    def predict(self, persons):
        """Gibt vorhergesagte Temperatur fuer Personenanzahl zurueck."""
    ...
    def get_status(self):
        return {
            ...
            "intercept": round(self.intercept, 2) if self.intercept is not None else None,
